...
navigable canvas ([Accessibility](./accessibility.md)). The last of those is better than either
...
| C6  | The schema's `tunneling-connection` type is **mislabeled**: its rendering (band + solid line + arrow) actually matches IBM's "Traffic Through Tunnel/Encapsulation" line, not the "Tunneling Connection" text — which `Connectors.drawio` stores as a `strokeColor=none` **caption** cell (`edge="0"`), not an edge, and has no line style of its own. Count stays at 11; this is a display-label correction, not a missing type. _(Corrected during implementation — the original audit misread the raster deck image; the stencil XML, a higher-precedence [normative source](./ibm-spec-conformance.md#normative-sources), settles it.)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `Connectors.drawio`: the two "Traffic Through …Tunnel/Encapsulation" cells are `style="text;…"` value labels on real edges; "Tunneling Connection" is a separate `fontSize=11` caption cell with no edge behind it |
...
Locked in [the decision log](../../../docs/decision-log.md#canvas--direct-manipulation) as D25–D28:

- **[D25](../../../docs/decision-log.md#d25--icons-render-as-ibm-authors-them-solid-tile-white-glyph--locked)** —
  icons render as IBM authors them: solid tile, white glyph.
- **[D26](../../../docs/decision-log.md#d26--gestures-are-ephemeral-commits-are-commands--locked)** —
  gestures are ephemeral; commits are commands.
- **[D27](../../../docs/decision-log.md#d27--the-interaction-state-machine-lives-in-core-not-the-shells--locked)** —
  the interaction state machine lives in core, not the shells.
- **[D28](../../../docs/decision-log.md#d28--constrained-defaults-full-range-on-demand--locked)** —
...
7. ✅ **Golden fixtures.** Not a `.drawio` parser — building one, even test-only, would cut against
   [D7](../../../docs/decision-log.md#d7--export-only-interop-svgpng-no-drawio-import--locked)'s locked "no
...
7. ✅ **Benchmark harness**, absorbing the intent of [M12](../../../docs/roadmap.md#m12--performance-at-scale)
...
Every gesture ships with its keyboard equivalent in the same PR — [D19](../../../docs/decision-log.md#d19--full-ibm-equal-access--wcag-21-aa--locked)
...
1. **Schema.** `BaseElement` gains `locked?: boolean` and `hidden?: boolean`
   ([`types.ts`](../src/scene/types.ts)) — purely additive optional fields, so no

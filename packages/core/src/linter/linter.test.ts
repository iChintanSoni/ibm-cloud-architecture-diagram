...
describe("Linter", () => {
  ...
  it("honors per-document severity overrides and disabled rules", () => {
    ...
    expect(new Linter().run(scene)[0]).toMatchObject({ ruleId: "missing-label", severity: "error" });
  ...
  it("flags duplicate labels and offers deterministic rename fixes", () => {
    ...
    const duplicates = new Linter().run(scene).filter((item) => item.ruleId === "duplicate-label");
  ...
  it("validates catalog references when a catalog is available", () => {
    ...
    expect(new Linter({ catalog }).run(scene)).toContainEqual(
      expect.objectContaining({ ruleId: "non-catalog-icon", severity: "error", elementId: "unknown" })
    );
  });
  ...
  it("normalizes off-spec primary fills and secondary strokes", () => {
    ...
    const diagnostics = new Linter().run(scene);
    diagnostics.find((item) => item.ruleId === "primary-color-fill")!.quickFix!.do(scene);
    diagnostics.find((item) => item.ruleId === "secondary-color-stroke")!.quickFix!.do(scene);

    expect(scene.get("vpc")?.style).toMatchObject({ fill: "#edf5ff", stroke: "#0f62fe" });
  });
  ...
  it("flags high-level nodes without location context", () => {
    ...
    expect(new Linter().run(scene)).toContainEqual(
      expect.objectContaining({ ruleId: "node-without-location", elementId: "app" })
    );
  });
  ...
  it("repairs invalid connector types and port bindings from hand-edited input", () => {
    ...
    const diagnostics = new Linter().run(scene);
    diagnostics.find((item) => item.ruleId === "non-standard-connector")!.quickFix!.do(scene);
    diagnostics.find((item) => item.ruleId === "connector-not-bound-to-port")!.quickFix!.do(scene);
  ...
  it("flags reversed public flow and icon geometry drift", () => {
    ...
    const diagnostics = new Linter().run(scene);
    expect(diagnostics).toContainEqual(expect.objectContaining({ ruleId: "west-east-flow-reversal" }));
    const geometry = diagnostics.find((item) => item.ruleId === "icon-geometry")!;
    geometry.quickFix!.do(scene);
    expect(scene.get("drifted")).toMatchObject({ w: 48, h: 48, style: { strokeWidth: 1 } });
  });
});

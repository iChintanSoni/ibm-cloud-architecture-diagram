...
export function applyQuickFix(diagnostic: Diagnostic): Command {
  if (!diagnostic.quickFix) throw new Error(`Diagnostic "${diagnostic.id}" has no quick-fix`);
...
export function applyQuickFixes(diagnostics: Diagnostic[], label = "fix validation issues"): Command {
  return batch(label, diagnostics.filter((item) => item.quickFix).map(applyQuickFix));
}

...
function loadRealCatalog(): Catalog {
  ...
  for (const icon of manifest.icons) {
    ...
    assets.set(icon.asset, raw.replace(/^<svg[^>]*>/, "").replace(/<\/svg>\s*$/, ""));
  }
  return new Catalog(manifest, assets);
}

function attributes(element: Element | null | undefined): Record<string, string> {
  if (!element) throw new Error("Missing golden-fixture element");
  return Object.fromEntries(Array.from(element.attributes).map((a) => [a.name, a.value]));
}
...
describe("golden fixture: iks_sr_mz_vpc reference structure", () => {
  ...
  it("renders the Client actor as a solid black circle with a white glyph, no outline (D25)", () => {
    ...
    expect(attributes(rect)).toMatchObject({ fill: "#000000", stroke: "none", rx: "24", ry: "24" });
  });
  ...
  it("renders every nesting level's own colored sidebar tab, at every depth (C7)", () => {
    ...
    const expectTab = (id: string, color: string) => {
      ...
      expect(attributes(rects![1] as Element)).toMatchObject({ width: "4", fill: color });
    };
  ...
  it("renders every container's corner glyph recolored to its own accent, since no tile sits behind it (D25)", () => {
    ...
    for (const [id, color] of [
      ...
      const glyphPath = renderer.nodeFor(id)?.querySelector(":scope > svg path, :scope > svg g");
      ...
      const recolored = renderer.nodeFor(id)?.querySelector(`[fill="${color}"]`);
      expect(recolored, `expected "${id}"'s corner glyph recolored to ${color}`).toBeTruthy();
    }
  });
  ...
  it("renders every catalog icon as a solid category-color tile with a white glyph, no outline (D25)", () => {
    ...
    const expectTile = (id: string, color: string) => {
      ...
      expect(attributes(glyphSvg)).toMatchObject({ width: "24", height: "24", viewBox: "0 0 24 24" });
    };
  ...
  it("renders public/private connection flow colors and IBM's 2px stroke width (C2–C5)", () => {
    buildScene();
    const line = (id: string) => renderer.nodeFor(id)?.querySelector("polyline");
    expect(attributes(line("client-to-nlb"))).toMatchObject({ stroke: "#4376BB", "stroke-width": "2" });
    expect(attributes(line("nlb-to-alb"))).toMatchObject({ stroke: "#198038", "stroke-width": "2" });
    expect(attributes(line("alb-to-worker-1"))).toMatchObject({ stroke: "#198038", "stroke-width": "2" });
    expect(attributes(line("alb-to-worker-2"))).toMatchObject({ stroke: "#198038", "stroke-width": "2" });
  });
});

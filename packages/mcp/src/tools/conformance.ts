...
import { z } from "zod";
import { recordDiagnostics, requireOpenDocument, ToolError, type ServerState } from "../state.js";
...
export function registerConformanceTools(server: McpServer, state: ServerState): void {
  server.registerTool(
    "lint",
    {
  ...
  server.registerTool(
    "quickfix_apply",
    {
      ...
      inputSchema: { diagnosticId: z.string().describe("A Diagnostic.id from the most recent lint() result") }
    },
    ({ diagnosticId }) => {
      try {
        requireOpenDocument(state);
        ...
        if (!diagnostic) {
          throw new ToolError(`Unknown or stale diagnostic id "${diagnosticId}" — call lint() again.`);
        }
        const applied = state.editor.applyQuickFix(diagnostic);
        if (!applied) throw new ToolError(`Diagnostic "${diagnosticId}" has no quick-fix.`);
        state.lastDiagnostics = new Map();
        return okText(`Applied the quick-fix for "${diagnosticId}". Call lint() again for the updated picture.`);
      } catch (err) {
        return fail(err);
      }
    }
  ...
  server.registerTool(
    "quickfix_apply_all",
    {
      ...
      description: "Applies every diagnostic's quick-fix as one undo step, optionally scoped to one rule.",
      inputSchema: { ruleId: z.string().optional().describe("Limit to one rule id; omit to fix everything fixable") },
  ...
  server.registerTool(
    "export_diagram",
    {
      ...
      inputSchema: {
        ...
        path: z.string().optional().describe("Write the SVG to this path instead of returning it inline")
      },
    ...
    async ({ embedSource, path: outPath }) => {
      try {
        requireOpenDocument(state);
        const svg = state.editor.export({ format: "svg", ...(embedSource !== undefined ? { embedSource } : {}) }) as string;
        if (outPath) {
          const resolved = resolvePath(outPath);
          await writeFile(resolved, svg, "utf-8");
          return ok({ path: resolved, bytesLength: svg.length }, `Wrote ${svg.length} bytes to "${resolved}".`);
        }
        return {
        ...
        if (err instanceof ExportBlockedError) {
          const summary = err.diagnostics
            ...
            .join("\n");
          return fail(new Error(`Export blocked by conformance errors:\n${summary}`));
        }
        return fail(err);
      }
    }

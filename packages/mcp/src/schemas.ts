...
export const connectorTypeSchema = z.union([connectionTypeSchema, relationshipTypeSchema]);

export const connectorDirectionSchema = z.enum(["unidirectional", "bidirectional"]);
...
export const catalogCategorySchema = z.object({ id: z.string(), name: z.string() });
...
export const placementSchema = {
  id: z.string().optional().describe("Explicit element id; auto-generated if omitted"),
  at: pointSchema.describe("Top-left scene-space position"),
  w: z.number().optional(),
  h: z.number().optional(),
  parentId: z.string().optional().describe("Containing Box/Group/Zone/Frame id"),
...
export const containerPlacementSchema = {
  ...placementSchema,
  catalogRef: z.string().optional().describe("Corner icon shown on the container, e.g. for a preset"),
...
const ruleCategorySchema = z.enum(["semantics", "containment", "labels", "connectors", "layout"]);

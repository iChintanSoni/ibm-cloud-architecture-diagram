...
function ensureUpstreamCheckout(): string {
  if (!existsSync(path.join(CACHE_DIR, ".git"))) {
    ...
    execFileSync("git", ["remote", "add", "origin", UPSTREAM_REPO], {
      cwd: CACHE_DIR,
    });
  }
  ...
  execFileSync(
    "git",
    ["fetch", "--quiet", "--depth", "1", "origin", UPSTREAM_REF],
    {
      cwd: CACHE_DIR,
      stdio: "inherit",
    },
...
function main() {
  ...
  const topFolders = readdirSync(svgRoot, { withFileTypes: true }).filter((e) =>
    e.isDirectory(),
  );
  ...
  for (const folder of topFolders) {
    ...
    if (!category) {
      if (!SKIPPED_FOLDERS.has(folder.name)) {
        console.warn(
          `Unrecognized upstream folder "${folder.name}" — skipping. Update categories.ts if this is new.`,
        );
      }
      continue;
    }
    ...
    const files = listSvgFiles(folderPath, folderPath).sort((a, b) =>
      a.rel.localeCompare(b.rel),
    );

    for (const file of files) {
      const skipSubpath = SKIPPED_SUBPATHS.some((sub) =>
        path.join(folder.name, file.rel).startsWith(sub),
      );
  ...
  const drawioLibraryPath = path.join(
    CACHE_DIR,
    "drawio",
    "stencils",
    "2.0",
    "not_released_in_drawio.xml",
  );
  let skippedGroupsDuped = 0;
  if (existsSync(drawioLibraryPath)) {
    for (const { title, normalized } of extractDrawioLibraryIcons(
      drawioLibraryPath,
    )) {
      ...
      const optimized = optimizeFragment(normalized.fragment, slug);
      const assetRelPath = path.posix.join(
        "icons",
        GROUPS_CATEGORY.id,
        `${slug}.svg`,
      );
  ...
  writeFileSync(
    path.join(CATALOG_ROOT, "index.json"),
  ...
  for (const icon of manifest.icons) {
    if (ids.has(icon.id))
      throw new Error(`Duplicate icon id emitted: ${icon.id}`);
    ids.add(icon.id);
    if (!icon.name.trim())
      throw new Error(`Icon ${icon.id} has no accessible name`);

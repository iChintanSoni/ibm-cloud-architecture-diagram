...
- **Consequence:** The core exposes an imperative API and event stream; shells never reach into
  core internals. See [Architecture](./architecture.md).
...
- **Why:** Privacy, git-friendliness, and zero infrastructure. Sharing = send the file or an
  exported SVG.
- **Consequence:** No real-time collaboration in v1 (revisit in [Roadmap](./roadmap.md)).
...
- **Consequence:** Opening a file requires the bundled catalog version it references; we pin and
  migrate catalog versions. See [File Format](../packages/core/docs/file-format.md).
...
- **Consequence:** Periodic re-sync when IBM ships new icons. See [Icon Catalog](../packages/catalog-build/docs/icon-catalog.md).
...
- **Consequence:** We maintain a rule set. See [Spec Conformance](../packages/core/docs/ibm-spec-conformance.md).
...
- **Consequence:** Skills version alongside the MCP toolset. See [Agent Integration](../packages/mcp/docs/agent-integration.md).
...
- **Consequence:** Canvas a11y is real, budgeted work. See [Accessibility](../packages/core/docs/accessibility.md).
...
- **Consequence:** The agent headline is designed-for but not shipped in v1. See [Roadmap](./roadmap.md).
...
- **Consequence:** [M3](./roadmap.md#m3--semantic-elements--containers) is unaffected — it ships
  the three primitives as already scoped. The preset table and its library-panel surfacing land in
  [M7](./roadmap.md#m7--chrome-templates-find-themes-carbon). See the proposed table in
  [Icon Catalog → Container presets](../packages/catalog-build/docs/icon-catalog.md#container-presets); several entries are
...
[M10](./roadmap.md#m10--appsvscode) replaced it with VS Code's hot exit. It mounts `apps/web`
...
- **Consequence:** `apps/web` gains a small number of Tauri-feature-detected code paths (native
  ...
  [Roadmap M11](./roadmap.md#m11--appsdesktop-tauri-shell).
...
- **Consequence:** `packages/catalog-build` gains a second extraction source
  ...
  [Icon Catalog → Build pipeline](../packages/catalog-build/docs/icon-catalog.md#build-pipeline).
...
[Container presets](../packages/catalog-build/docs/icon-catalog.md#container-presets) table. Direct evidence — the sidebar-tab
...
[docs/05](../packages/core/docs/ibm-spec-conformance.md#element-semantics)'s own worked-example text ("a virtual
...
- **Consequence:** `ZoneKind` narrows to `az | on-prem`; `packages/core/src/templates/templates.ts`'s
  ...
  [Element semantics](../packages/core/docs/ibm-spec-conformance.md#element-semantics).

## Canvas & Direct Manipulation

Adopted while planning [Canvas parity](../packages/core/docs/canvas-parity-plan.md), after auditing the engine against
...
- **Consequence:** `rotation` and `canvas.grid` stop being dead schema fields. Rotation lands last
  ...
  [M20](../packages/core/docs/canvas-parity-plan.md#m20--full-range-on-demand)**: all four items (rotation gesture,
...
A `.icad` file pins `catalog: { id, version }` ([File Format](../packages/core/docs/file-format.md#top-level-shape)),
separate from the document's own integer `version` (the schema field `io/migrations`' `MIGRATIONS`
registry is keyed by, [File Format → Versioning](../packages/core/docs/file-format.md#versioning--migration)). When a
...
[Icon Catalog](../packages/catalog-build/docs/icon-catalog.md#catalog-manifest-schema)) — already consulted by
...
- **Why:** [Icon Catalog → Versioning strategy](../packages/catalog-build/docs/icon-catalog.md#versioning-strategy) already
  ...
  ([M13](./roadmap.md#m13--catalog-refresh-cadence--migration-tooling)) called this out explicitly
...
- **Availability Zone corner icon:** the source's Zone containers carry a corner glyph, but no
  ...
  presets](../packages/catalog-build/docs/icon-catalog.md#container-presets) already flags this gap, same shape as
...
- **Consequence:** `packages/mcp`'s `doc_create` gains the 4 new ids in `diagramTemplateIdSchema`
  ...
  See [Roadmap M26](./roadmap.md#m26--reference-architecture-templates).
...
- **Why:** `packages/mcp` stays a thin, dependency-light MCP server any MCP client can point at —
  ...
  ([M21](./roadmap.md#m21--licensing--release-packaging)) for every consumer, not just this one.
...
- **Why:** [Agent Integration](../packages/mcp/docs/agent-integration.md) already named `GenerateArchitectureDiagram`
...
- **Amended during real M30 live-model testing:** `export_diagram`/`doc_save` (plus the agent-side
  ...
  [Roadmap M30](./roadmap.md#m30--deep-agent-orchestrator--sub-agents) for the complete list of
...
- **Why:** The MCP server holds exactly one open document for its whole process lifetime
  ([AI agents & MCP](../packages/mcp/docs/ai-agents-mcp.md)) — fresh-per-task is the simplest correct mapping,
...
- **Why:** A file path matches ICAD's local-first, shared-filesystem assumption
  ...
  ([M9.1's deferral](./roadmap.md#m91--catalog-document-authoring-and-conformancesvg-export-tools)) —
...
- **Consequence:** `packages/mcp`'s `export_diagram` tool and its SVG-only limitation are untouched
  ...
  confirmed at [M31](./roadmap.md#m31--agent-side-png-export) kickoff) instead. Remote or
...
- **Consequence:** "The agent remembers my conventions across sessions" is explicit future scope,
  ...
  [M30](./roadmap.md#m30--deep-agent-orchestrator--sub-agents).

...
> **v6 feature — not yet built.** Design locked via [D31](00-decision-log.md#d31--new-agent-package-hosts-the-deep-agent-runtime-kept-separate-from-the-mcp-server--locked-v6)–[D37](00-decision-log.md#d37--hard-export-gate-auto-fix-everything-fixable-then-block-on-remaining-errors--locked-v6);
> [Roadmap v6](09-roadmap.md#v6--autonomous-agent-runtime-deep-agents--a2a) has the milestone
> breakdown this document assumes.

`apps/agent` is the concrete "Solution Architecture Agent" [Agent Integration](08-agent-integration.md)
already sketched: a standalone process that takes a natural-language requirement (or a
natural-language edit instruction plus an existing `.icad`), drives `packages/mcp`'s 25-tool
authoring surface end-to-end, and hands back a diagram — with no human in the tool loop. Read
[Agent Integration](08-agent-integration.md) first for the MCP server and Agent Skills this builds
...
[M9.2](09-roadmap.md#m92--agent-skills) shipped the three `SKILL.md` packages an agent framework
...
2. `apps/agent`'s `AgentExecutor.execute()` spawns a fresh `packages/mcp` stdio subprocess for this
   task alone ([D34](00-decision-log.md#d34--one-ephemeral-mcp-subprocess-per-task-single-task-at-a-time--locked-v6))
...
6. On a clean lint pass: `export_diagram({ format: "svg" })`, then an agent-side SVG→PNG conversion
   step ([D35](00-decision-log.md#d35--existing-diagrams-are-referenced-by-file-path-png-is-produced-agent-side--locked-v6)),
   then `doc_save()`.
...
boundary the three skills already assume ([D33](00-decision-log.md#d33--orchestrator-plus-two-sub-agents-diagram-builder-and-conformance-exporter--locked-v6)),
...
- **Client capability:** wired via the SDK's `ClientFactory`, unused beyond the dev-harness CLI
  ([M33](09-roadmap.md#m33--a2a-dev-harness-cli-plumbing-only-a2a-client-dogfooding)) — no real
...
config file) is decided at [M30](09-roadmap.md#m30--deep-agent-orchestrator--sub-agents) kickoff.
...
See [Roadmap → v6](09-roadmap.md#v6--autonomous-agent-runtime-deep-agents--a2a) for the M29–M33

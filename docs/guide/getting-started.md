...
![New diagram dialog](../screenshots/new-diagram-dialog.png)
...
![Populated canvas showing a Customer actor connected through an API Gateway, an Application box inside an Application tier group, inside a VPC box, inside an IBM Cloud box, to Object storage](../screenshots/hero-canvas-overview.png)
...
- Drag new icons in from the **Library** panel on the left (see
  [The web editor](../../apps/web/docs/web-editor.md#placing-icons) — placement is click-to-place, not
  drag-and-drop).
...
see **[The web editor](../../apps/web/docs/web-editor.md)**.
...
- **VS Code extension** — edit `.icad` files inside VS Code. Install the `.vsix` from Releases (no
  Marketplace listing yet), or build from source. See [The VS Code extension](../../apps/vscode/docs/vscode-extension.md).
- **Desktop app** — a native Tauri shell with real file associations and OS dialogs. Download the
  ...
  or build from source. See [The desktop app](../../apps/desktop/docs/desktop-app.md).
- **AI agents / MCP** — point an MCP-capable agent at `packages/mcp` to author diagrams
  ...
  [AI agents & MCP](../../packages/mcp/docs/ai-agents-mcp.md).

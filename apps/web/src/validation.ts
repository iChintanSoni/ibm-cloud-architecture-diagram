...
export function buildValidationView(diagnostics: Diagnostic[], exportGate: ExportGate): ValidationView {
  const counts: Record<Severity, number> = { error: 0, warn: 0, info: 0 };
  const fixableByRule = new Map<string, number>();
  for (const diagnostic of diagnostics) {
    counts[diagnostic.severity] += 1;
    if (diagnostic.quickFix) {
      fixableByRule.set(diagnostic.ruleId, (fixableByRule.get(diagnostic.ruleId) ?? 0) + 1);
    }
  }
  ...
  return {
    groups: SEVERITY_ORDER.map((severity) => ({
      severity,
      items: diagnostics.filter((diagnostic) => diagnostic.severity === severity)
    })),
    counts,
    fixableByRule,
